 ...
 * Standard implementation with 256-entry edge/triangle lookup tables.
 */
...
// Triangle table: for each configuration, up to 5 triangles defined by edge indices (-1 terminates)
 ...
 * @param data - flat Float32Array of nx*ny*nz values (x-major order)
 * @param nx, ny, nz - grid dimensions
 * @param origin - [x,y,z] of grid origin
 * @param extent - [dx,dy,dz] total size
...
export function marchingCubes(
  ...
  const dx = extent[0] / (nx - 1);
  const dy = extent[1] / (ny - 1);
  const dz = extent[2] / (nz - 1);
  ...
  function interp(
    p1: number[], p2: number[], v1: number, v2: number
  ): number[] {
  ...
  for (let ix = 0; ix < nx - 1; ix++) {
    for (let iy = 0; iy < ny - 1; iy++) {
      for (let iz = 0; iz < nz - 1; iz++) {
        ...
        const vals = [
          data[idx(ix,iy,iz)], data[idx(ix+1,iy,iz)],
          data[idx(ix+1,iy+1,iz)], data[idx(ix,iy+1,iz)],
          data[idx(ix,iy,iz+1)], data[idx(ix+1,iy,iz+1)],
          data[idx(ix+1,iy+1,iz+1)], data[idx(ix,iy+1,iz+1)],
        ];
        ...
        for (let e = 0; e < 12; e++) {
          if (EDGE_TABLE[cubeIndex] & (1 << e)) {
            ...
            vertList[e] = interp(pts[a], pts[b], vals[a], vals[b]);
          }
        }
        ...
        for (let t = 0; t < tri.length; t += 3) {
          if (tri[t] === -1) break;
          const v0 = vertList[tri[t]];
          const v1 = vertList[tri[t+1]];
          const v2 = vertList[tri[t+2]];
